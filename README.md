...
- **[Hermes Agent](https://github.com/NousResearch/hermes-agent)** `⭐ 245k` — Nous Research's self-improving CLI agent with persistent memory, automated skill creation, sandboxed code execution via Unix socket RPC, and multi-platform reach (Telegram/Slack/Discord/WhatsApp); supports 300+ models across multiple providers.

- **[OpenCode](https://github.com/anomalyco/opencode)** `⭐ 207k` — Terminal-native coding agent with 75+ provider support, LSP integration, and privacy-first design (formerly opencode-ai; now at opencode.ai).
...
- **[Codex CLI](https://github.com/openai/codex)** `⭐ 124k` `[OpenAI]` — OpenAI's local coding agent for reading/editing/running code, with an interactive TUI and tool execution. Apache-2.0.
...
- **[Continue CLI](https://github.com/continuedev/continue)** `⭐ 35.9k` — Open-source terminal extension for multi-model coding with local/privacy focus.
...
- **[Kilo Code CLI](https://github.com/Kilo-Org/kilocode)** `⭐ 27.3k` — Agentic engineering platform with CLI; orchestrator mode, 100s of LLMs, skills, and checkpointing.

- **[Grok Build](https://github.com/xai-org/grok-build)** `⭐ 26.7k` `[xAI]` — xAI's official coding agent harness and TUI; fullscreen, mouse-interactive, and extensible. Apache-2.0.
...
- **[MiMo Code](https://github.com/XiaomiMiMo/MiMo-Code)** `⭐ 13.1k` `[Xiaomi]` — Xiaomi's official terminal coding agent; TUI + non-interactive modes, MCP, skills, hooks, git worktrees, and resumable sessions. Runs MiMo-V2.5-Pro or any mainstream provider; npm `@xiaomi-mimo/cli`. MIT.
...
- **[Letta Code](https://github.com/letta-ai/letta-code)** `⭐ 3.3k` — Memory-first CLI coding agent built on the Letta platform (formerly MemGPT); persistent memory across sessions, model-agnostic (Claude/GPT/Gemini), skill learning, and context repositories.
...
- **[BitFun](https://github.com/GCWing/BitFun)** `⭐ 2.1k` — Rust coding agent with an interactive TUI and non-interactive `bitfun exec` runs; session fork/undo/redo/compact with a timeline, MCP and ACP, skills, hooks, and daemon-backed background tasks. Ships alongside, but builds independently of, a Tauri desktop app. MIT.
...
- **[Keen Code](https://github.com/mochow13/keen-code)** `⭐ 62` — Go-based CLI coding agent focused on efficient context management — uses lean TurnMemory summaries instead of raw tool traces to maximize context efficiency. 9+ providers, skill-driven MCP servers, and full transparency with every prompt, design doc, and implementation plan saved as markdown. MIT.
...
- **[Jazz](https://github.com/lvndry/jazz)** `⭐ 51` — Generalist agent harness that runs your agent anywhere: terminal, scripts, cron, a GitHub Action that reviews PRs, or a Telegram/Discord bot you own. Define model, persona, toolset, and permissions in one JSON file; 18 providers (incl. local Ollama/llama.cpp) plus MCP, with permission-gated file, git, and web tools that ask for approval wherever you are. MIT.

- **[Smelt](https://github.com/leonardcser/smelt)** `⭐ 42` — Rust TUI coding agent; multi-provider (Anthropic, OpenAI, Ollama, GitHub Copilot, any OpenAI-compatible endpoint), four modes (Normal/Plan/Apply/Yolo), granular permission system, parallel subagents, vim keybindings, and headless scriptable mode. MIT.
...
- **[Forge (Norvia Labs)](https://github.com/NorviaLabs/forge)** `⭐ 8` — Rust TUI coding agent unifying an AI agent, a vim-style code editor, and a shell in one keyboard-driven workspace; approval-aware command execution, durable SQLite session journal, and MCP server support. MIT.
...
- **[nanobot](https://github.com/HKUDS/nanobot)** `⭐ 48.1k` — Ultra-lightweight ~4,000-line Python rewrite of OpenClaw; tool use, persistent memory, scheduled tasks, and multi-channel support (Telegram/Discord/WhatsApp). MIT.
...
- **[GitClaw](https://github.com/open-gitagent/gitclaw)** `⭐ 679` — Git-native AI agent framework where agent identity, rules, memory, tools, and skills are all version-controlled files. MIT.
...
- **[Claude Code](https://github.com/anthropics/claude-code)** `⭐ 145k` `[Anthropic]` — Anthropic's repo-aware terminal agent for code edits, refactors, and git workflows. Source-available, no OSS license.

- **[Warp](https://github.com/warpdotdev/Warp)** `⭐ 65k` `[Warp]` — Modern terminal with built-in AI agent mode; understands tasks, runs commands, edits files, and orchestrates multi-step workflows.
...
- **[vibe-kanban](https://github.com/BloopAI/vibe-kanban)** `⭐ 28.1k` — Kanban interface for administering AI coding agents.
...
- **[Emdash](https://github.com/generalaction/emdash)** `⭐ 5.7k` — Run multiple coding agents concurrently with coordinated workflows.
...
- **[agents-cli](https://github.com/phnx-labs/agents-cli)** `⭐ 20` — CLI to install, version-pin, and run many coding-agent harnesses (Claude Code, Codex, Gemini CLI, Cursor, OpenCode, Grok); shared skills/MCP/rules, parallel teams in isolated terminals, session index, and SSH fleet dispatch. npm `@phnx-labs/agents-cli`. Apache-2.0.

- **[repomon](https://github.com/AliHamzaAzam/repomon)** `⭐ 20` — Run a fleet of AI coding agents (Claude Code, Codex, Aider) across many repos, branches, and git worktrees from one tmux-backed terminal. Four-zoom TUI (fleet, split, babysit grid, focus), needs-you triage, durable sessions that survive restarts.
...
- **[LoopTroop](https://github.com/looptroop-ai/LoopTroop)** `⭐ 136` — Local, open-source GUI orchestrator for AI coding agents. An LLM Council plans, atomic "beads" execute in isolated git worktrees, and a "Ralph Loop" retries failures with fresh context to fight context rot. Built on OpenCode. MIT.

- **[kodo](https://github.com/ikamensh/kodo)** `⭐ 132` — Autonomous multi-agent coding orchestrator that directs Claude Code, Codex, and Gemini CLI through work cycles with independent architect and tester verification. SWE-bench verified.

- **[wreckit](https://github.com/mikehostetler/wreckit)** `⭐ 131` — Apply the Ralph Wiggum Loop pattern across your roadmap for autonomous agent execution.

- **[great_cto](https://github.com/avelikiy/great_cto)** `⭐ 93` — Engineering-management layer of 34 specialist AI agents covering the full SDLC (architect, PM, senior-dev, reviewer, QA, security, devops, L3-support + 18 archetype-specific reviewers) with auto-detected archetypes and compliance gates (PCI-DSS, HIPAA, FedRAMP, GDPR, EU AI Act). Runs in Claude Code, Cursor, Codex CLI, Aider, and Continue via AGENTS.md + MCP. MIT.
...
- **[MartinLoop](https://github.com/Keesan12/martin-loop)** `⭐ 51` — Execution-control layer around Claude Code, Codex, and other coding agents: token and cost budgets, iteration caps, stop conditions, independent verifier passes, and signed run receipts, driven from a `martin` CLI. Apache-2.0.

- **[Agon](https://github.com/AutoResearch-Factory/Agon)** `⭐ 48` — Claude Code plugin for autonomous research: runs a topic→idea→proposal→experiment loop with separate scientist, coder, and auditor roles.
...
- **[NEEDLE](https://github.com/jedarden/NEEDLE)** `⭐ 25` — Headless bead-queue orchestrator built on an explicit deterministic state machine: claims work items and dispatches each to a headless coding-agent CLI (Claude Code, Codex, OpenCode, Aider), with a defined handler for every outcome rather than a model deciding the next step. Rust, MIT.

- **[ralph-harness](https://github.com/rxdt/py_ralph_frame)** `⭐ 21` — Minimal repo-local loop scaffold for Claude Code, Codex CLI, and Gemini CLI. Uses `PROMPT.md`, specs, fresh-context iterations, git hooks, CI verification, and hard iteration/time caps so agents make small gated commits instead of drifting in one long chat. MIT.

- **[Ordewell](https://github.com/ordewell/ordewell)** `⭐ 19` — Turns one goal into an ordered plan of coding-agent tasks, each with its own runner (Claude Code, Codex, OpenCode), model, and mode; a terminal UI drives planning, execution, and verification. TypeScript.
...
- **[agent-qa](https://github.com/vostride/agent-qa)** `⭐ 907` — Self-improving QA harness for web and mobile workflows, with a CLI, dashboard, MCP and skills for coding agents, memory-backed self-healing, and sandboxed hooks for setup/teardown. FSL-1.1-ALv2.
...
- **[agenttrace](https://github.com/luoyuctl/agenttrace)** `⭐ 131` — Local-first Bubble Tea TUI for inspecting Claude Code, Codex CLI, Gemini CLI, Aider, Cursor, Hermes, OpenCode, Kimi, and Copilot-style session logs; surfaces cost, cache usage, failures, latency, anomalies, health gates, and diffs.
...
- **[ax](https://github.com/Necmttn/ax)** `⭐ 109` — Local-first agent telemetry and observability for Claude Code and Codex sessions.
...
- **[skill-optimizer](https://github.com/fastxyz/skill-optimizer)** `⭐ 79` — CLI tool that benchmarks SDK, CLI, and MCP guidance docs (SKILL.md) across multiple LLMs using static action + argument matching. Iteratively rewrites docs until every configured model meets a PASS/FAIL score floor. MIT.
...
- **[AgentManager](https://github.com/kevinelliott/agentmanager)** `⭐ 35` — Lightweight CLI for managing multiple agent runs/sessions and workflows.

- **[Loadout](https://github.com/elleryfamilia/loadout)** `⭐ 30` — Adaptive context engine for AI coding agents; detects your stack and equips the right context when you launch `load claude`, `load codex`, `load cursor`. Works with Claude, Codex, Cursor, opencode, and Copilot. Rust, MIT.
...
- **[Agentic Engineering Framework](https://github.com/DimitriGeelen/agentic-engineering-framework)** `⭐ 14` — Provider-neutral governance framework for CLI coding agents. Structural enforcement of task-driven workflows, context budget management, antifragile healing loops, and audit compliance. Works with Claude Code, Aider, Cursor, and any file-based agent.
...
- **[Okto Nexus](https://github.com/OktoLabsAI/okto-nexus)** `⭐ 12` — Local-first coordination hub for agents working in the same repository: durable identities, presence, messages and inboxes, handoffs, artifacts, an event log, and governance controls, backed by one SQLite database with no cloud broker. `okto-nexus serve` exposes MCP over streamable HTTP, operator REST APIs, and a bundled dashboard on one port. Python.
